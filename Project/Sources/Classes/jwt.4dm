...
jwt.sign( payloadObject ; header) -> tokenString
	
header.alg: a JWT algorithm ES256, ES384, RS256, HS256, etc...
text : password for HS@ algorithms
*/
Function sign($payload_obj : Object; $header_obj : Object; $secret : Text)->$token : Text
	C_TEXT:C284($header; $payload; $signature; $hash)
	$header_obj.typ:=$header_obj.typ || "JWT"
	$header_obj.alg:=$header_obj.alg || "ES256"
	
	BASE64 ENCODE:C895(JSON Stringify:C1217($header_obj); $header; *)
	BASE64 ENCODE:C895(JSON Stringify:C1217($payload_obj); $payload; *)
	$signature:=""
	$hash:=This:C1470._hashFromAlgorithm($header_obj.alg)
	
	Case of 
			
		: ($header_obj.alg="ES@") | ($header_obj.alg="RS@") | ($header_obj.alg="PS@")
			// need a private key
			If (Asserted:C1132(This:C1470.key#Null:C1517))
				$signOptions:=New object:C1471("hash"; $hash; "pss"; $header_obj.alg="PS@"; "encoding"; "Base64URL")
				$signature:=This:C1470.key.sign($header+"."+$payload; $signOptions)
			End if 
			
		: ($header_obj.alg="HS@")
			C_TEXT:C284($secret)
			$secret:=($secret="") ? String:C10(This:C1470.secret) : $secret
...
Function verify
	...
	C_TEXT:C284($token; $header; $payload; $signature; $hash; $alg)
	C_TEXT:C284($headerDecoded; $payloadDecoded)
	C_LONGINT:C283($pos1; $pos2)
	C_OBJECT:C1216($headerObject; $payloadObject; $options)
